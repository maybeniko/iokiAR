...
open class LocationAnnotationNode: LocationNode {
    ...
    public init(location: CLLocation?, image: UIImage) {
        ...
        annotationNode = AnnotationNode(view: nil, image: image, coordinate: location?.coordinate)
    ...
    public init(location: CLLocation?, view: CustomView, cornerRadius: CGFloat) {
        let plane = SCNPlane(width: view.frame.size.width / 200, height: view.frame.size.height / 200)
        plane.firstMaterial!.diffuse.contents = view
        plane.firstMaterial!.lightingModel = .constant
        plane.cornerRadius = cornerRadius

        annotationNode = AnnotationNode(view: view, image: nil, coordinate: location?.coordinate)
        annotationNode.geometry = plane
