...
open class AnnotationNode: SCNNode {
    ...
    public init(view: CustomView?, image: UIImage?, coordinate: CLLocationCoordinate2D?) {
        super.init()
        self.view = view
        self.image = image
        ...
        if locationConfirmed && (distance > 100 || continuallyAdjustNodePositionWhenWithinRange || setup) {
            ...
            if distance < 30 {
                ...
                self.position = SCNVector3( x: position.x + Float(locationTranslation.longitudeTranslation),
                                            y: position.y + Float(locationTranslation.altitudeTranslation + 7.5),
                                            z: position.z - Float(locationTranslation.latitudeTranslation))
                self.scale = SCNVector3(x: 1.5, y: 1.5, z: 1.5)
                ...
                let adjustedTranslation = SCNVector3( x: Float(locationTranslation.longitudeTranslation) * scale,
                                                      y: Float(locationTranslation.altitudeTranslation + 23.0) * scale,
                ...
                adjustedDistance = distance
                self.position = SCNVector3( x: position.x + Float(locationTranslation.longitudeTranslation),
                                            y: position.y + Float(locationTranslation.altitudeTranslation + 26.0),
                                            z: position.z - Float(locationTranslation.latitudeTranslation))
                ...
                self.position = SCNVector3( x: position.x + Float(locationTranslation.longitudeTranslation),
                                            y: position.y + Float(locationTranslation.altitudeTranslation + 7.5),
                                            z: position.z - Float(locationTranslation.latitudeTranslation))
                self.scale = SCNVector3(x: 1.2, y: 1.2, z: 1.2)
            }
        } else {
