...
public class PolylineNode {
    ...
    private let lightNode: SCNNode = {
        ...
        node.light!.attenuationEndDistance = 600
    ...
    fileprivate func contructNodes() {
        ...
        for i in 0 ..< polyline.pointCount - 1 {
            ...
            let box = SCNBox(width: 2, height: 0.2, length: CGFloat(distance), chamferRadius: 0)
            box.firstMaterial?.diffuse.contents = UIColor(red: 47/255.0, green: 83/255.0, blue: 255/255.0, alpha: 1.0)

            let bearing = -currentLocation.bearing(between: nextLocation)

            let boxNode = SCNNode(geometry: box)
            boxNode.pivot = SCNMatrix4MakeTranslation(0, 15, 0.5 * Float(distance))
