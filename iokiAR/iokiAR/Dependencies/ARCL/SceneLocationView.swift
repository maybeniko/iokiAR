...
public extension SceneLocationView {
    ...
    func addRoutes(routes: [MKRoute]) {
        ...
        print(routes.count)
        let polyNodes = routes.map { PolylineNode(polyline: $0.polyline, altitude: altitude - 30.0) }
        print("drawing")
    ...
    //draw polyline with ARRoute
    func addAugmentedRoute(coordinates: [CLLocationCoordinate2D]) {
        guard let altitude = sceneLocationManager.currentLocation?.altitude else {
            return assertionFailure("we don't have an elevation")
        }
        ...
        let polyNodes = MKPolyline(coordinates: coordinates, count: coordinates.count)
        
        let poly = PolylineNode(polyline: polyNodes, altitude: altitude - 30)
        polylineNodes.append(poly)
        
        poly.locationNodes.forEach {
            $0.updatePositionAndScale(setup: true,
                                      scenePosition: currentScenePosition,
                                      locationManager: sceneLocationManager,
                                      onCompletion: {})
